package com.studycode.poi.read;
...
public class ExcelXlsReader implements HSSFListener {
    ...
    // 起始读取行数
    private Integer beginParseIndex = 0;

    // 读取多少条数据
    private Integer countParseIndex = 0;
     ...
     * @throws Exception
     */
    public int process(String fileName, int begin, int count) throws Exception {
        filePath = fileName;
        this.beginParseIndex = begin;
        this.countParseIndex = count;
    ...
    public void processRecord(Record record) {
        ...
        if (record instanceof LastCellOfRowDummyRecord) {
            ...
            if (flag) { //该行不为空行且该行不是第一行，发送（第一行为列名，不需要）
                if (curRow == 0) {
                    // TODO: 2022/6/27 这里是头信息，需要的话这里可以处理
                    ...
                    if (curRow >= beginParseIndex) {
                        ...
                        // TODO: 2022/6/25 这里根据情况处理
                        ExcelReaderUtil.sendRows(filePath, sheetName, sheetIndex, curRow + 1, cellList); //每行结束时，调用sendRows()方法

                        readCount++;
                        if (readCount >= countParseIndex && countParseIndex >=0) {
                            // TODO: 2022/6/27 自定义中断异常，捕捉该异常
                             throw new RuntimeException("解析完成，中断了！");
                        }
                    }
